...
public class EquipeDAO {
    ...
    private List<Artisan> getArtisansForEquipe(Connection conn, int equipeId) throws SQLException {
        String sql = "SELECT u.id, u.nom, a.* " +
                "FROM utilisateur u " +
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    // Création avec données de User
                    Artisan artisan = new Artisan(
                            rs.getInt("artisan_id"), // ID de User
                            rs.getString("nom")   // Nom de User
                    );
    ...
    private Equipe mapEquipe(ResultSet rs ,Connection conn) throws SQLException {
        ...
        // Mapping du constructeur
        Constructeur constructeur = new Constructeur();
        constructeur.setConstructeur_id(rs.getInt("constructeur_id"));
        // Si besoin de plus de détails, utiliser constructeurDAO.findById
        equipe.setConstructeur(constructeur);

        // Mapping du gestionnaire de stock
        GestionnaireDeStock gestionnaireStock = new GestionnaireDeStock();
        gestionnaireStock.setGestionnairestock_id(rs.getInt("gestionnairestock_id"));
        // Si besoin de plus de détails, utiliser gestionnaireStockDAO.findById
        equipe.setGestionnaireStock(gestionnaireStock);

        // Mapping des artisans (optionnel ici, souvent fait séparément)
