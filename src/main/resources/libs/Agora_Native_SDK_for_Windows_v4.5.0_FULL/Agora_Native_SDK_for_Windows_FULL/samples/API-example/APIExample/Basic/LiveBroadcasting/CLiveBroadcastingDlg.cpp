...
void CLiveBroadcastingRtcEngineEventHandler::onVideoRenderingTracingResult(uid_t uid, MEDIA_TRACE_EVENT currentEvent, VideoRenderingTracingInfo info)
{
	if (m_hMsgHanlder)
	{
		...
		*i = info;
		::PostMessage(m_hMsgHanlder, WM_MSGID(EID_VIDEO_RENDERING_TRACING_RESULT), (WPARAM)i, uid);
	}
}
...
CLiveBroadcastingDlg::CLiveBroadcastingDlg(CWnd *pParent /*=nullptr*/)
	: CDialogEx(IDD_DIALOG_LIVEBROADCASTING, pParent), m_canvasRenderModeValue(0)
{
}
...
void CLiveBroadcastingDlg::DoDataExchange(CDataExchange *pDX)
{
	...
	DDX_Control(pDX, IDC_SLIDER_Local_fps, mLocalFpsSlider);
	DDX_Control(pDX, IDC_SLIDER_Remote_fps, mRemoteFpsSlider);
}
...
ON_MESSAGE(WM_MSGID(EID_VIDEO_RENDERING_TRACING_RESULT), &CLiveBroadcastingDlg::onEIDVideoRenderingTracingResult)
...
ON_NOTIFY(NM_CUSTOMDRAW, IDC_SLIDER_Local_fps, &CLiveBroadcastingDlg::OnNMCustomdrawSliderLocalfps)
ON_NOTIFY(NM_CUSTOMDRAW, IDC_SLIDER_Remote_fps, &CLiveBroadcastingDlg::OnNMCustomdrawSliderRemotefps)
END_MESSAGE_MAP()
...
void CLiveBroadcastingDlg::InitCtrlText()
{
	...
	mLocalFpsSlider.SetRange(1, 60);
	mLocalFpsSlider.SetPos(15);
	mRemoteFpsSlider.SetRange(1, 60);
	mRemoteFpsSlider.SetPos(15);
}
...
void CLiveBroadcastingDlg::ResumeStatus()
{
	...
	m_chkFirstFrameOpt.EnableWindow(TRUE);
...
LRESULT CLiveBroadcastingDlg::onEIDVideoRenderingTracingResult(WPARAM wParam, LPARAM lParam)
{
	VideoRenderingTracingInfo *info = (VideoRenderingTracingInfo *)wParam;
	uid_t uid = lParam;

	CString strInfo;
	strInfo.Format(_T("===VideoRenderingTracingResult==="));
	m_lstInfo.InsertString(m_lstInfo.GetCount(), strInfo);

	strInfo.Format(_T("uid=%d"), uid);
	m_lstInfo.InsertString(m_lstInfo.GetCount(), strInfo);

	strInfo.Format(_T("elapsedTime=%dms"), info->elapsedTime);
	m_lstInfo.InsertString(m_lstInfo.GetCount(), strInfo);

	strInfo.Format(_T("start2JoinChannel=%dms"), info->start2JoinChannel);
	m_lstInfo.InsertString(m_lstInfo.GetCount(), strInfo);

	strInfo.Format(_T("join2JoinSuccess=%dms"), info->join2JoinSuccess);
	m_lstInfo.InsertString(m_lstInfo.GetCount(), strInfo);

	strInfo.Format(_T("joinSuccess2RemoteJoined=%dms"), info->joinSuccess2RemoteJoined);
	m_lstInfo.InsertString(m_lstInfo.GetCount(), strInfo);

	strInfo.Format(_T("remoteJoined2SetView=%dms"), info->remoteJoined2SetView);
	m_lstInfo.InsertString(m_lstInfo.GetCount(), strInfo);

	strInfo.Format(_T("remoteJoined2UnmuteVideo=%dms"), info->remoteJoined2UnmuteVideo);
	m_lstInfo.InsertString(m_lstInfo.GetCount(), strInfo);

	strInfo.Format(_T("remoteJoined2PacketReceived=%dms"), info->remoteJoined2PacketReceived);
	m_lstInfo.InsertString(m_lstInfo.GetCount(), strInfo);
...
void CLiveBroadcastingDlg::OnBnClickedCheckFirstFrameOpt()
{
	if (m_chkFirstFrameOpt.GetCheck())
	{

		UINT ret = MessageBox(liveBraodcastingFristFrameTipContent, liveBraodcastingFristFrameTip, MB_YESNO | MB_ICONQUESTION);
		if (ret == IDYES)
		{
			m_rtcEngine->enableInstantMediaRendering();
			m_chkFirstFrameOpt.EnableWindow(FALSE);
		}
		else
		{
			m_chkFirstFrameOpt.SetCheck(FALSE);
...
void CLiveBroadcastingDlg::OnNMCustomdrawSliderLocalfps(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMCUSTOMDRAW pNMCD = reinterpret_cast<LPNMCUSTOMDRAW>(pNMHDR);
	*pResult = 0;
	if (!m_rtcEngine || !m_initialize)
		return;
	int localfps = mLocalFpsSlider.GetPos();
	CString localfpsStr;
	localfpsStr.Format(_T("local fps %d"), localfps);
	m_lstInfo.InsertString(m_lstInfo.GetCount(),localfpsStr);
	m_rtcEngine->setLocalRenderTargetFps(VIDEO_SOURCE_CAMERA,localfps);
}

void CLiveBroadcastingDlg::OnNMCustomdrawSliderRemotefps(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMCUSTOMDRAW pNMCD = reinterpret_cast<LPNMCUSTOMDRAW>(pNMHDR);
	*pResult = 0;
	if (!m_rtcEngine || !m_initialize)
		return;
	int remotefps = mRemoteFpsSlider.GetPos();
	CString remotefpsStr;
	remotefpsStr.Format(_T("local fps %d"), remotefps);
	m_lstInfo.InsertString(m_lstInfo.GetCount(),remotefpsStr);
	m_rtcEngine->setRemoteRenderTargetFps(remotefps);
}
